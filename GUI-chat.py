#GUI-chat.py
from tkinter import *
from tkinter import ttk, messagebox # import to create GUI
...
GUI.title('Program Chat Uncle')

FONT1 = ('Angsana New',15) # font no.1
FONT2 = ('Angsana New',17) # font no.2
###### chat box #######
F1 = Frame(GUI) # create frame
F1.place(x=5,y=5) # define frame follow axis x and y

allmsg = StringVar() # record massage

chatbox = st.ScrolledText(F1,width=53,heigh=15,font=FONT1)
chatbox.pack(expand=True, fill='x')
###### message form #######
v_msg = StringVar()

F2 = Frame(GUI) # create frame
F2.place(x=5,y=450) # define frame follow axis x and y

E1 = ttk.Entry(F2,textvariable=v_msg,font=FONT2,width=35)
E1.pack(ipady=5)
...
def SendMessage(event=None):
	...
	client.sendall(msg.encode('utf-8'))  # send message to server
	chatbox.delete(1.0,END) # clear old msg
	chatbox.insert(INSERT, allmsg.get()) # insert new
	chatbox.yview(END) 
	v_msg.set('') # clear msg
...
############### Run Server ###########################
		...
		try: # try connect to server
				...
				client.sendall(firsttext.encode('utf-8'))
				task = threading.Thread(target=server_hanler, args=(client,)) # create threading
				task.start()
		except:
				print('ERROR!')
				messagebox.showerror('Connection Failed','ไม่สามารถเชื่อมต่อ server ได้')
